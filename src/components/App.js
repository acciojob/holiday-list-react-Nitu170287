...
const App = () => {
  ...
  const indianCityList = cityList.filter(city => city.country === "India");
  console.log(indianCityList);
  return (
    <div id="main">
      ...
      <ol >
        {
          indianCityList.map((city,index)=><li key={`location${index+1}`}>{city.name}</li>)
        }
      </ol>
    </div>
  );
